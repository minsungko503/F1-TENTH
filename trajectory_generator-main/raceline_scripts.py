...
def smooth_waypoints(filename="race_v1.csv", gap_thres=0.4, interpolate_times=2):
    if filename:
        ...
        with open(os.path.join(cur_csv_dir, filename)) as f:
            ...
            for i, row in enumerate(waypoints):
                ...
                if i > 1:
                    ...
                    if abs(x - x_old) + abs(y - y_old) > globalWpMinInterval:
                        if (
                            np.linalg.norm(np.array([x, y]) - np.array([x_old, y_old]))
                            > gap_thres
                        ):
                            interp_points = [
                                np.array([x_old, y_old, r_old, l_old]),
                                np.array([x, y, r, l]),
                            ]
                            for _ in range(interpolate_times):
                                ...
                                n = len(interp_points)
                                while i < n:
                                    interp_points.insert(
                                        i + 1,
                                        (interp_points[i] + interp_points[i + 1]) / 2,
                                    )
                                    i += 2
            ...
            # import ipdb; ipdb.set_trace()
            if (
                np.linalg.norm(np.array([x_old, y_old]) - np.array(first_point)[:2])
                > gap_thres
            ):
                interp_points = [
                    np.array([x_old, y_old, r_old, l_old]),
                    np.array([first_point]),
                ]
                for _ in range(interpolate_times):
                    i = 0
                    n = len(interp_points)
                    while i < n:
                        interp_points.insert(
                            i + 1,
                            (interp_points[i] + interp_points[i + 1]).squeeze() / 2,
                        )
                        i += 2
...
def get_smooth_lane(
    shrink_dist=0.2, option="center", original_wp="centerline.csv", backup_exist=True
):
    wp_x = []
    wp_y = []
    wp_r = []
...
def draw_lanes(
    lane_l="lane_left.csv", lane_r="lane_right.csv", lane_center="lane_center.csv"
):
    xs, ys = [], []
    ...
    for x, y, draw_opt, label in zip(
        xs, ys, ("-bo", "-ro", "-ko"), ("left", "right", "center")
    ):
...
def visualize_curvature_for_wp(wpfile="lane_optimal.csv", ka_thres=0.05):
    ...
    po = np.array(
        [[wp_x[0], wp_y[1]]] + po + [[wp_x[-2], wp_y[-2]]] + [[wp_x[-1], wp_y[-1]]]
    )
    ...
    segments = []
    while i < n_ka:
        if abs(ka[i]) <  3 * ka_thres:
            begin = i
            while i < n_ka and abs(ka[i]) < 3* ka_thres:
                i += 1
            if i - begin > 20:
    ...
    segments = []
    while i < n_ka:
        if abs(ka[i]) > 5 * ka_thres:
            begin = i
            while i < n_ka and abs(ka[i]) > 5 * ka_thres:
                i += 1
            if i - begin > 10:
                segments.append((begin, i - 1))
        else:
    ...
    plt.scatter(
        ...
        np.array(slow_wp)[:, 1],
        c="g",
        label="slowdown_segment",
        s=5,
    )
    plt.scatter(
        np.array(overtake_wp)[:, 0],
        np.array(overtake_wp)[:, 1],
        c="r",
        label="overtaking_segment",
        s=5,
    )
    plt.axis("equal")
    plt.legend()
